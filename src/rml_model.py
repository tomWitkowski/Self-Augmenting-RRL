...
class SharpeRatio(tf.keras.layers.Layer):
    ...
    def __init__(self, lewar: int = 1, with_swap: bool = False, reward_type: str = 'sharpe'):
...
class Encoder(tf.keras.Model):
    ...
    def dense(x, units, activation='relu', dropout=0.2):
        x = tf.keras.layers.Dense(units, kernel_regularizer='l2')(x)
        x = tf.keras.layers.BatchNormalization()(x)
    ...
    def __init__(self, x_shape, model_config):
        ...
        """
        # super().__init__()
        layer = in_data = tf.keras.Input(shape=(x_shape))
        # layer = self.norm(in_data)
...
class Decoder(tf.keras.Model):
    ...
    @staticmethod
    def dense(x, units, activation='relu', dropout=0.2, use_batch_norm:bool=True):
        x = tf.keras.layers.Dense(units, kernel_regularizer='l2')(x)
        if use_batch_norm:
            x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Activation(activation)(x)
...
class Agent:
    ...
    @staticmethod
    def randomly_remove_n_percent(xv, ba, percent):
        """
        Returns arrays which skips first percent of the data.
        """
        N = int(len(xv) * percent)
        # sample na between 0 and N
        n = random.randint(0, N)
        return xv[n:], ba[n:]


    def add_noise(self, xv, std):
        """
        Impose random (normal) noise 
        """
        noise = np.random.normal(0, std, xv.shape)
        return xv + noise
    ...
    def fit(self, XV, BA, epochs, verbose=0):
        ...
        for e in range(epochs):
            xv_, ba_ = self.randomly_remove_n_percent(xv, ba, 0.05)
            xv_ = self.add_noise(xv_, 0.01)

            grads, decisions, rewards, loss_value = self.train_iteration(xv_, ba_
                                                                            )
...
def build_regime_switch(merged):
    ...
    # two logits → softmax → take first entry as p(regime=1)
    logits = tf.keras.layers.Dense(
                2,
                kernel_initializer=tf.keras.initializers.RandomNormal(stddev=1e-2),
                bias_initializer=tf.keras.initializers.Zeros()
             )(x)
    probs = tf.keras.layers.Activation('softmax', name='regime_probs')(logits)
    # final scalar in [0,1]:
    regime_switch = tf.keras.layers.Lambda(lambda z: z[:, 0:1], name='regime_switch')(probs)

    return regime_switch



class RegimeSwitchingDecoder(tf.keras.Model):
    """
    M_D: merges latent input + previous action to produce the next action.
    """
    @staticmethod
    ...
    @staticmethod
    def self_attention(x):
        # Expand and flatten input for attention layer
        if len(x.shape) == 2:
    ...
    def __init__(self, layer_shape, model_config: dict):
        # super().__init__()
        z_in = tf.keras.Input(shape=(layer_shape,))
        another_decision = tf.keras.Input(shape=(1,))
        prev_action_in = tf.keras.Input((3,))
        layer = merged = tf.concat([z_in, prev_action_in, another_decision], 1)
        ...
        # regime_switch = self.dense(merged, units=10, activation='relu')
        # regime_switch = self.dense(regime_switch, units=5, activation='relu')
        # regime_switch = self.dense(regime_switch, units=1, activation='sigmoid', use_batch_norm=False)

        regime_switch = build_regime_switch(merged)

        def _capture(x):
            # when the model runs, this Lambda will be called with the
            # *actual* tensor value, and we stash it on the instance
            self._regime_output = x
            return x
        regime_switch = tf.keras.layers.Lambda(_capture)(regime_switch)

        out = tf.keras.layers.Dense(1)(layer)
        out = tf.keras.activations.tanh(out)

        dec = regime_switch * out + (1-regime_switch) * another_decision

        super().__init__(
            inputs  = [z_in, prev_action_in, another_decision],
            outputs = [dec],
            name    = "RegimeSwitchingDecoder"
        )

        super().__init__(inputs=[z_in, prev_action_in, another_decision], outputs=[dec])
        self.compile(optimizer='Adam', loss='mse', metrics=[self.monitor_loss])
...
class RegimeSwitchingAgent:
    """
    High-level API combining Encoder, Decoder, and UtilityFunction.
    Switches between the model prediction and the baseline.
    """
    def __init__(self, encoder=None, decoder=None, utility_function=None, input_len=None):
        if encoder is None:
            self.encoder = Encoder(input_len)
        ...
        if decoder is None:
            self.decoder = RegimeSwitchingDecoder(self.encoder.output.shape[1])
        else:
            if not isinstance(decoder, RegimeSwitchingDecoder):
                raise ValueError("Decoder must be an instance of RegimeSwitchingDecoder.")
            self.decoder = decoder
    ...
    def multiply_decisions(self, ZV, MACD):
        if len(ZV) != int(len(self._phi_X)/3):
            zeros = tf.zeros((len(ZV),1))
            ones = tf.ones((len(ZV),1))
            self._phi_actions = tf.concat([
            ...
            self._phi_X = tf.concat([ZV, ZV, ZV], 0)
            self._phi_MACD = tf.concat([MACD, MACD, MACD], 0)
        return self._phi_X, self._phi_actions, self._phi_MACD
    ...
    def train_iteration(self, XV, BA, MACD):
        """
        Performs a single training iteration for the model.
        Args:
            XV (tf.Tensor): Input features tensor for the encoder.
            BA (tf.Tensor): Batch of BID, ASK
            MACD (tf.Tensor): Not used in the current implementation, reserved for future use.
        Returns:
            tuple:
                - ge_gd (tf.Tensor): Sum of encoder and decoder gradients after applying them.
                - dec_seq (tf.Tensor): Sequence of decisions derived from phi_seq.
                - reward (tf.Tensor): Computed reward from the utility function.
                - loss_value (tf.Tensor): Loss value (negative reward) used for optimization.
        Notes:
            This method performs a forward pass through the encoder, processes the latent representation,
            computes predictions, and applies the utility function to obtain the reward and loss.
            Gradients are computed and applied to update model parameters.
        """
        with tf.GradientTape() as tape:
            z_out = self.encoder(XV)
            stacked_z, stacked_a, stacked_macd = self.multiply_decisions(z_out, MACD)
            stacked_preds = self.decoder([stacked_z, stacked_a, stacked_macd])
            phi_seq = self.phi_processing(stacked_preds)
            final_dec = self.decoder([z_out, phi_seq, MACD])
            reward = self.utility_function([BA, final_dec])
            loss_value = -reward
        ge,gd = self.compute_apply_grads(tape, loss_value)
        dec_seq = tf.argmax(phi_seq,1)-1
        return ge+gd, tf.round(final_dec), reward, loss_value

    def test_iteration(self, XV, BA, MACD, batch_size=10000,  just_historical_path=False):
        z_out = self.encoder.predict(XV, batch_size=batch_size, verbose=0)
        if len(z_out.shape) == 1:
            z_out = np.expand_dims(z_out, 1)
        z_out_tf = tf.constant(z_out, tf.float32)
        stacked_z, stacked_a, stacked_macd = self.multiply_decisions(z_out_tf, MACD)
        stacked_preds = self.decoder([stacked_z, stacked_a, stacked_macd])
        phi_seq = self.phi_processing(stacked_preds)
        if just_historical_path:
            return phi_seq[:-1]
        dec_seq = tf.argmax(phi_seq,1)-1
        decoder_pred = self.decoder.predict([z_out, phi_seq, MACD], batch_size=batch_size, verbose=0)
        reward = self.utility_function([BA, tf.round(decoder_pred)])
        loss_value = -reward
        return [], tf.round(decoder_pred), reward, loss_value, decoder_pred
    ...
    def fit(self, XV, BA, MACD, epochs, verbose=0):
        xv = XV.values
        ba = BA.values
        macd = MACD.values
        for e in range(epochs):
            grads, decisions, rewards, loss_value = self.train_iteration(xv, ba, macd
                                                                            )
            grads = [sum([x.numpy().reshape(-1,).tolist() for x in grads],[])]
